...
define(['utils/Constant'], function (Constant) {
  ...
  var LoginController = function ($scope, $log, LoginSvc, CommonSvc, $cookies, $location, $rootScope, $timeout, localStorageService, $interval) {
    ...
    if ($scope.loginTkDomain) {
      LoginSvc.loginTk({
        ...
        if (response.respCode === '_200') {
          //$location.url(response.result.gotoUrl);
    ...
    CommonSvc.getDomains(function (response) {
      var result = Constant.transformResponse(response);
      if (result === undefined) {
        $scope.loginFailed = true;
        $scope.loginAlertContent = Constant.loadError;
        $scope.domains = [];
        $scope.domain = {};
        return;
      }
      if (!result || !result.length) {
        ...
        $scope.domain = {};
        $scope.loginFailed = true;
        $scope.loginAlertContent = Constant.loadEmpty;
        return;
      }
      $scope.selectedDomain = response.result[0];
      var i, len = response.result.length;
      $scope.domains = response.result;
      for (i = 0; i < len; i++) {
        if ($scope.loginTkDomain === $scope.domains[i].name) {
    ...
    //$scope.alertContent = '用户名/登陆密码错误，请重新登录';
    ...
    $scope.submitResetPwd = function () {
      /**  var myRegExp = /[a-z0-9-]{1,30}@[a-z0-9-]{1,65}.[a-z]{3}/;
        if (!myRegExp.test($scope.resetPwdEmail)) {
          $scope.resetPwdAlertContent = '您输入的邮箱格式有误';
          $scope.resetPwdFailed = true;
        } else if ($scope.resetPwdCaptcha == undefined) {
    ...
    /** Login when user click login button. */
